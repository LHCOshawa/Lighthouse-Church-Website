/* ============================================================
   LIGHTHOUSE CHURCH — BACKEND SERVER
   Google Calendar API v3 via Service Account
   ============================================================ */
...
function getAuthClient() {
  // Load from service-account.json in project root
...
app.get('/api/events', async (req, res) => {
  try {
    ...
    const events = items.map(item => {
      ...
      // Extract image from description (looks for <img> or a direct URL)
      ...
      return {
        ...
        month:       startDate.getMonth(),      // 0-indexed
        day:         startDate.getDate(),
        weekday:     startDate.getDay(),         // 0 = Sun
      };
    });
...
/* ── START ────────────────────────────────────────────────── */
app.listen(PORT, () => {
  console.log(`\n✦ Lighthouse Church server running on http://localhost:${PORT}`);
